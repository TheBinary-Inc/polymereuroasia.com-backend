...
search_products.post("/search", verify_admin, async (req, res) => {
  ...
  if (search_product) {
    try {
      const searchedProducts = await AllProducts.find();
      let resultPro = searchedProducts
        .filter((i) => {
          return i.name.toUpperCase().includes(search_product.toUpperCase());
        })
        .slice(0, 10);
      res.status(200).json({
        message: "Successfully searched!",
        search_results: resultPro,
      });
    } catch (err) {
      res.status(500).json({
...
search_products.post("/searchmain", async (req, res) => {
  const { search_product } = req.body;
  if (search_product) {
    try {
      const searchedProducts = await AllProducts.find();
